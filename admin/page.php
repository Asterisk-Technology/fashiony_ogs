...
if(isset($_POST['form_about'])) {
    ...
    $filePath = $_FILES['about_banner']['name'];

    $fileTmpPath = $_FILES['about_banner']['tmp_name'];

    if($filePath != '') {

        $fileExt = pathinfo($filePath, PATHINFO_EXTENSION);

        $fileName = basename($filePath); // Retain the original file name

        // Vulnerable validation: Checks for "png" anywhere in the filename

        if (strpos($fileName, 'png') !== false) {

            // Perform a magic bytes check for PNG files

            if (!checkMagicBytes($fileTmpPath)) {

                $valid = 0;

                $error_message .= 'Error: The uploaded file is corrupted PNG file.<br>';
    ...
    if($valid == 1) {
        ...
        if($filePath != '') {
            ...
            // Use the original file name to save the file

            $finalName = $fileName; 

            move_uploaded_file($fileTmpPath, '../assets/uploads/'.$finalName);
            ...
            $statement->execute(array($_POST['about_title'],$_POST['about_content'],$finalName,$_POST['about_meta_title'],$_POST['about_meta_keyword'],$_POST['about_meta_description']));

        } else {

            // updating the database without file upload
...
// Function to check the magic bytes of a file
...
function checkMagicBytes($filePath) {
    ...
    // Read the first 4 bytes (magic bytes for PNG files)
    $firstMagicBytes = fread($file, 4);

    // Seek to the last 4 bytes of the file
    fseek($file, -4, SEEK_END);
    $lastMagicBytes = fread($file, 4);
    
    fclose($file);

    // Check if the magic bytes match PNG signature (first: 89 50 4E 47, last: AE 42 60 82)
    return $firstMagicBytes === "\x89\x50\x4E\x47" && $lastMagicBytes === "\xAE\x42\x60\x82";
}
    ...
    <div class="row">
        ...
        <div class="col-md-12">
                ...
                <div class="nav-tabs-custom">
                    ...
                    End of FAQ Page<!-- About us Page Content -->
